...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'VB:Rio_\n' // NAMA OWNER BOT
            + 'ORG:Creator Venix Bot;\n' // KASIH CREDITS GUA FADHIL GRAPHY SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=6283101195576+62 831-011-95576\n' // GANTI NOMOR LU
            + 'END:VCARD'
prefix = '.'
blocked = []
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285859219397@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["1 (715) 355 1080 (@s.whatsapp.net"] // GANTI NOMOR LU
			...
			case 'owner':
                 ...
                 client.sendMessage(from, 'itu pembuatku :) ><',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
               ...
               case 'infogempa':
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('sabar ya')
					if (Number(args[0]) === 1) {
